        ...
        @"
            ...
            column_definitions AS (
                ...
                WHERE
                    ...
                    AND table_name = (SELECT table_name FROM table_info)
            ),
            constraints AS (
                SELECT
                    tc.constraint_type,
            ...
            LEFT JOIN
                constraints c ON cd.column_name = c.column_name
            ;
        ";
    ...
    internal const string FETCH_TABLES_CONSTRAINS =
        @"
            ...
            WHERE
                ...
                AND c.conrelid = '{{schema_name}}.{{table_name}}'::regclass
            ORDER BY
                c.conname
        ";
